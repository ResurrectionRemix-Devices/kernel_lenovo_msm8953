...
static void epl_sensor_report_lux(int report_lux)
{
	...
	input_report_rel(epld->als_input_dev, SYN_TIME_SEC, ktime_to_timespec(timestamp).tv_sec);
	input_report_rel(epld->als_input_dev, SYN_TIME_NSEC, ktime_to_timespec(timestamp).tv_nsec);
...
static void epl_sensor_report_ps_status(void)
{
	...
	else
	{
		...
		input_report_abs(epld->ps_input_dev, ABS_DISTANCE, ps_status_moto);
	}
	...
	input_report_rel(epld->ps_input_dev, SYN_TIME_SEC, ktime_to_timespec(timestamp).tv_sec);
	input_report_rel(epld->ps_input_dev, SYN_TIME_NSEC, ktime_to_timespec(timestamp).tv_nsec);
	input_sync(epld->ps_input_dev);
}
...
#if ESD_PATCH
    if((enable_als==1 && epl_sensor.als.polling_mode==1) || (enable_ps==1))
#else
...
static void epl_sensor_polling_work(struct work_struct *work)
{
    ...
    cancel_delayed_work(&epld->polling_work);
#if ESD_PATCH
    if( (enable_als &&  epl_sensor.als.polling_mode == 1) || (enable_ps == 1) )
#else
    ...
    if(enable_ps == 1)
    {
        ...
        {
            esd_flag = true;
            LOG_INFO("[%s]: ESD happened...... REG0x05=0x%x, REG0x06 = 0x%x \r\n", __func__, buf[0], buf[1]);
	...
	if(epl_sensor.ps.interrupt_flag == EPL_INT_TRIGGER) {
		...
		if ((epl_sensor.ps.compare_low >> 3) == 0) {
			if (read_h_thd == ps_thd_3cm) {
				ps_status_moto = 3;
				set_psensor_intr_threshold(ps_thd_5cm, ps_thd_1cm);
				mutex_lock(&sensor_mutex);
		...
		ps_status_moto = 100;
		if((read_h_thd == ps_thd_1cm) || (read_h_thd == ps_thd_3cm))
			set_psensor_intr_threshold(ps_thd_5cm, ps_thd_3cm);
		}
		if(enable_ps) {
...
static DEVICE_ATTR(ps_enable,				S_IWUSR | S_IRUGO, epl_sensor_show_ps_enable,			epl_sensor_store_ps_enable					);
...
static int sensor_power_set(struct epl_sensor_priv *data, bool on)
{
	...
	if (!on && data->power_enabled) {
		...
		if (rc) {
			dev_err(&data->client->dev, "Regulator i2c_l6 disable failed rc=%d\n", rc);
			rc = regulator_enable(data->vdd);
			dev_err(&data->client->dev, "Regulator i2c_l6 re-enabled rc=%d\n", rc);
			/*
...
static int sensor_power_init(struct epl_sensor_priv *data, bool on)
{
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->i2c_l6) > 0)
			regulator_set_voltage(data->i2c_l6, 0,SENSOR_I2C_L6_MAX_UV);
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd,
				SENSOR_VDD_MIN_UV, SENSOR_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->client->dev,
		...
		if (regulator_count_voltages(data->i2c_l6) > 0) {
			rc = regulator_set_voltage(data->i2c_l6,
				SENSOR_I2C_L6_MIN_UV, SENSOR_I2C_L6_MAX_UV);
			if (rc) {
				dev_err(&data->client->dev,
